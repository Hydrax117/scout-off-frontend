...
export function readReceiptsEnabled(): boolean {
  ...
  const stored = window.localStorage.getItem(NOTIFICATION_PREF_READ_RECEIPTS_KEY);
  return stored === null ? true : stored === 'true';
}

export function setReadReceiptsEnabled(enabled: boolean): void {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem(NOTIFICATION_PREF_READ_RECEIPTS_KEY, String(enabled));
}
