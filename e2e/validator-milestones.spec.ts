...
test.describe('validator approve / revoke milestone', () => {
  test('approves a milestone, verifies progress, then revokes and verifies revert', async ({
    ...
    await expect(page.getByText('Test Player')).toBeVisible({ timeout: 10_000 });

    await page.getByRole('button', { name: 'Select' }).click();

    await page.getByLabel(/milestone description/i).fill('Test milestone approval');
    ...
    await page.getByLabel(/milestone description/i).fill('Test milestone revocation');
    await page.getByLabel(/evidence url/i).fill('https://example.com/evidence-revoke');
